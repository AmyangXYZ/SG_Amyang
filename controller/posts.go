...
func Get(ctx *sweetygo.Context) {
	...
	if page := ctx.Param("page"); page != "" {
		if cat := ctx.Param("cat"); cat != "" {
			posts, err := models.GetPostsByCat(cat, page)
			if err != nil {
			...
			data := map[string][]models.Post{"posts": posts}
			ctx.JSON(200, data, "success")
			return
		}

		posts, err := models.GetPosts(page)
		if err != nil {
			ctx.JSON(500, "Get Posts Error", "error")
			return
		}
...
func New(ctx *sweetygo.Context) {
	...
	text := ctx.Param("text")
	if title != "" && cat != "" && text != "" {
		err := models.NewPost(title, cat, text)
		if err != nil {
...
func Update(ctx *sweetygo.Context) {
	...
	text := ctx.Param("text")
	if title != "" && cat != "" && text != "" {
		err := models.UpdatePost(title, cat, text)
		if err != nil {
